...
namespace App.Areas.Identity.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        )
        {
            ...
            model.Claims = await _context
                .RoleClaims.Where(rc => rc.RoleId == role.Id)
                .ToListAsync();
            model.role = role;
            ModelState.Clear();
            return View(model);
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditConfirmAsync(
            string roleid,
            [Bind("Name")] EditRoleModel model
        )
        {
        ...
        public async Task<IActionResult> AddRoleClaimAsync(string roleid)
        {
            ...
            var model = new EditClaimModel() { role = role };
            return View(model);
        }
        ...
        public async Task<IActionResult> AddRoleClaimAsync(
            string roleid,
            [Bind("ClaimType", "ClaimValue")] EditClaimModel model
        )
        {
            if (roleid == null)
                return NotFound("Không tìm thấy role");
            ...
            if (
                (await _roleManager.GetClaimsAsync(role)).Any(c =>
                    c.Type == model.ClaimType && c.Value == model.ClaimValue
                )
            )
            {
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EditRoleClaim(
            int claimid,
            [Bind("ClaimType", "ClaimValue")] EditClaimModel Input
        )
        {
            var claim = _context.RoleClaims.Where(c => c.Id == claimid).FirstOrDefault();
            if (claim == null)
