...
namespace App.Areas.Identity.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        )
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if ((externalEmailUser != null) && (registeredUser == null))
                {
                    ModelState.AddModelError(
                        string.Empty,
                        "Không hỗ trợ tạo tài khoản mới - có email khác email từ dịch vụ ngoài"
                    );
                    return View();
                }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            {
                return RedirectToAction(
                    nameof(AccountController.ResetPasswordConfirmation),
                    "Account"
                );
            }
        ...
        public async Task<IActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            if (!ModelState.IsValid || string.IsNullOrWhiteSpace(model.SelectedProvider))
            {
                // repopulate lại Providers vì nó KHÔNG được submit lên cùng model
                var userFactors = await _userManager.GetValidTwoFactorProvidersAsync(user);
                model.Providers = userFactors
                    .Select(p => new SelectListItem { Text = p, Value = p })
                    .ToList();
                ModelState.AddModelError(string.Empty, "Vui lòng chọn phương thức nhận mã.");
                return View(model); // trước đây là "return View();" -> cũng là 1 bug, mất luôn cả Providers lẫn model
            }
            ...
            {
                await _emailSender.SendSmsAsync(
